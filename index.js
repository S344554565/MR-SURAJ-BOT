...
console.log = (...args) => {
  const msg = args.join(' ');
  if (msg.toLowerCase().includes('mqtt')) return;
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(usersPath)) {
        const usersData = fs.readJsonSync(usersPath);
        configData.ADMIN_USERNAME = usersData.ADMIN_USERNAME || 'admin';
        configData.ADMIN_PASSWORD = usersData.ADMIN_PASSWORD || 'admin';
      }
    } catch (e) {
      configData.ADMIN_USERNAME = 'admin';
      configData.ADMIN_PASSWORD = 'admin';
    }
    return configData;
  } catch (e) {
    return {
...
global.reloadChand = async function() {
  try {
    ...
    Object.keys(require.cache).forEach(key => {
      if (key.includes(__dirname) && (key.includes('CHAND') || key.includes('fca') || key.includes('cookieManager') || key.includes('lodash-pari') || key.includes('schedulers'))) {
        try {
          delete require.cache[key];
        } catch(e) {}
      }
...
app.get('/api/status', (req, res) => {
  ...
  // Check cookies validity if no account issue file exists
  if (!accountIssue) {
    try {
      const CookieManager = require('./Data/system/cookieManager');
      const cookieStatus = CookieManager.getCookiesStatus();
      if (!cookieStatus.valid) {
        accountIssue = true;
        // Save account issue for tracking
        fs.writeFileSync(path.join(__dirname, 'Data/system/account_issue.txt'), 'INVALID_COOKIES|' + cookieStatus.reason);
      }
    } catch (e) {}
  }
  ...
  try {
    ...
    if (fs.existsSync(botInfoPath)) {
      ...
      botName = botInfo.name || global.config?.BOTNAME || '';
    }
  } catch (e) {}
...
app.post('/api/login', (req, res) => {
  try {
    ...
    try {
      const usersPath = path.join(__dirname, 'users.json');
      const usersData = fs.readJsonSync(usersPath);
      if (username === usersData.ADMIN_USERNAME && password === usersData.ADMIN_PASSWORD) {
...
app.post('/api/change-credentials', (req, res) => {
  try {
    const { username, password } = req.body;
    const usersPath = path.join(__dirname, 'users.json');
    const usersData = { ADMIN_USERNAME: username, ADMIN_PASSWORD: password };
